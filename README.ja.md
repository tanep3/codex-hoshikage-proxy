...
- クライアント定義のツール呼び出し／結果・usageの完全なOpenAI形式変換は未対応です。質問・MCP elicitation・権限専用承認は、対応を宣言したv2クライアント向けの[対話中継API](docs/interaction-api.ja.md)を実装しています（Gateway UI・結合受入は未完了）。未対応のツール指定は受付前に拒否し、未宣言・未対応の対話要求は理由を返して対象Turnの停止を要求します。
