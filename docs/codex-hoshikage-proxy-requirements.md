...
### MCP確認待ち時間（2026-09-17訂正）

内部カタログの30秒TTLを利用者の確認期限にしてはならない。同一定義への正常更新では既存画面の承認証拠を維持する。更新中／失敗は機械可読な再照会状態とし、秘密判定・未評価への見かけの変更で補わない。本人向け入口を含む全状態を接続型で検証する。具体契約は[API 0.6第14節](mcp-approval-api-v06.ja.md)。
