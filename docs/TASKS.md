...
画像detail=lowの不一致は未解消。全自動テストは76件（Unit 41、HTTP 28、Runtime 5、Supervisor 2）で、Clippyも警告なし。
...
- 詳細なAPI・制約・責務境界は[契約回答](control-api.ja.md)。2026-09-11に`6875bb0`を常駐サービスへ適用済み。認証・実生成・要求照会・Turn状態・重複防止を確認した。[適用記録](server-operations.md#2026-09-11-制御api-v1適用記録)を参照。
