...
本書は接続合意済み0.3の正本。0.3は実装・接続試験用の常駐反映済みで、実Discordの総合受入とは区別する。[配備記録](server-operations.md)を参照。

**公開カードで操作内容と承認をまとめる改訂は別文書の[0.4接続レビュー案](mcp-inline-approval-api.ja.md)を参照。** 0.4は未合意・未実装。0.3のargumentsを公開可能へ変更したものではない。以下の0.3の型・秘密保持条件は現行実装の基準として維持する。
