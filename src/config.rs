...
impl ValidatedConfig {
    ...
    pub fn from_raw(raw: RawConfig) -> Result<Self, ConfigError> {
        ...
        if !matches!(
            raw.codex.sandbox.mode.as_str(),
            "read-only" | "workspace-write"
        ) {
            return Err(ConfigError::Invalid(
                "codex.sandbox.mode must be read-only or workspace-write".into(),
            ));
        }
        if raw.codex.sandbox.mode != "workspace-write"
            && !raw.codex.sandbox.writable_roots.is_empty()
        {
            return Err(ConfigError::Invalid(
                "codex.sandbox.writable_roots requires workspace-write mode".into(),
            ));
        }
        ...
        for configured in raw.codex.sandbox.writable_roots {
            let path = PathBuf::from(expand_home(&configured));
            if !path.is_absolute() {
                return Err(ConfigError::Invalid(format!(
                    "sandbox writable root must be absolute: {path:?}"
                )));
            }
            let canonical = path.canonicalize().map_err(|error| {
                ConfigError::Invalid(format!(
                    "sandbox writable root does not exist: {path:?}: {error}"
                ))
            })?;
            if !roots.iter().any(|root| canonical.starts_with(root)) {
                return Err(ConfigError::Invalid(format!(
                    "sandbox writable root is outside security.allowed_cwds: {canonical:?}"
                )));
            }
            sandbox_writable_roots.push(canonical);
        }
        ...
        Ok(Self {
            ...
            sandbox_mode: raw.codex.sandbox.mode,
            sandbox_writable_roots,
            sandbox_network_access: raw.codex.sandbox.network_access,
        })
    }
    ...
    pub fn prepare_codex_home(&self) -> Result<(), ConfigError> {
        ...
        if self.sandbox_mode == "workspace-write" {
            ...
            if !self.sandbox_writable_roots.is_empty() {
                let roots = self
                    .sandbox_writable_roots
                    .iter()
                    .map(|root| toml_string(&root.to_string_lossy()))
                    .collect::<Vec<_>>()
                    .join(", ");
                content.push_str(&format!("writable_roots = [{roots}]\n"));
            }
        }
        content.push('\n');
...
mod tests {
    ...
    fn generates_codex_provider_config_without_touching_auth() {
        ...
        raw.codex.sandbox.network_access = true;
        ...
        assert!(generated.contains("sandbox_mode = \"workspace-write\""));
        assert!(generated.contains("network_access = true"));
        assert!(generated.contains("writable_roots = [\"/tmp\"]"));
    ...
    fn rejects_sandbox_writable_root_outside_cwd_allowlist() {
        ...
        let error = ValidatedConfig::from_raw(raw).unwrap_err();
        assert!(
            matches!(error, ConfigError::Invalid(message) if message.contains("outside security.allowed_cwds"))
        );
    }
